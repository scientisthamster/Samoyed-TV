package com.scientisthamsterssofiandjohn.samoyedtv.view.ui.top_rate
...
import com.scientisthamsterssofiandjohn.samoyedtv.databinding.FragmentTopRateBinding
...
class TopRateFragment : Fragment(R.layout.fragment_top_rate), SwipeRefreshLayout.OnRefreshListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        topRateViewModel =
            ViewModelProvider(this, viewModelFactory).get(TopRateViewModel::class.java)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        binding = FragmentTopRateBinding.bind(view)
        initUI()
        obsStates()
        obsData()
    }
    ...
    private fun obsData() {
        topRateViewModel.moviePagedLiveData.observe(viewLifecycleOwner, Observer { pagedList ->
            pagedAdapter.submitList(pagedList)
        })
    }

    private fun obsStates() {
        topRateViewModel.paginationState?.observe(viewLifecycleOwner, Observer {
    ...
    override fun onRefresh() {
        topRateViewModel.refreshAllList()
    }

    override fun onClickRetry() {
        topRateViewModel.refreshFailedRequest()
    }

    override fun onMovieClick(movieResult: MovieResult, position: Int) {
        val bundle = bundleOf("movie" to movieResult)
        findNavController().navigate(R.id.action_topRateFragment_to_detailFragment, bundle)
    }
